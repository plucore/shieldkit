...
* `vercel.json` defines **5 Vercel Cron jobs** (was 4; `measure-page-speed` added 2026-07-30). Lowest-frequency-allowed slot on Hobby is daily, so sub-daily cadence lives in GitHub Actions with the Vercel entry as failsafe.
...
- `20260803120000` `plan_selection_shown_at` (2026-08-03 — one-time plan-picker marker; **backfilled to now() on every existing row** so the redirect applies to NEW installs only)
- `20260730163104` `set_generated_policy` (2026-07-30 batch 2 — single-key policy writer; local file `20260730160000_*`)
...
| `plan_selection_shown_at` | TIMESTAMPTZ | **2026-08-03.** When the merchant was redirected to Shopify's hosted plan picker. NULL = never shown → the `app.tsx` layout loader redirects once and stamps this. Backfilled to `now()` on all 60 existing rows at migration time, so only new installs are redirected. To opt the existing free base in: `UPDATE merchants SET plan_selection_shown_at = NULL WHERE tier='free' AND uninstalled_at IS NULL`. |
...
### Function: `set_generated_policy(p_merchant_id UUID, p_type TEXT, p_body TEXT, p_mark_regen_used BOOLEAN DEFAULT false)` (2026-07-30)
Writes **ONE** policy key via `jsonb_set` against the current row. Replaced a read-modify-write that spread the whole `generated_policies` object from a read at the top of the action and wrote it back wholesale — so two concurrent generations of **different** policy types both started from the same base and the second silently dropped the first. `useSingleFlight` does not cover it: that guard is per-button, and "generate refund" and "generate shipping" are different buttons.

`p_mark_regen_used` additionally sets `policy_regen_used[type]` in the SAME statement, folding in the degraded fallback that used to write both columns non-atomically from two stale in-memory objects (a second lost-update site).

**Does NOT replace `finalize_policy_regen`.** That one carries the conditional guard deciding who *wins* a concurrent regen (zero rows to the loser, output discarded); this one is the unconditional single-key writer. **Keep both** — swapping one for the other would let two regens both succeed.
...
9. `page_speed` (**info, PERMANENTLY NON-SCORABLE, measured OFF the scan** — 2026-07-30) — Google PageSpeed Insights mobile score + full-screen pop-up ("intrusive interstitial") detection. **The scan makes no PSI call at all**: it writes a `pendingPageSpeed()` placeholder and enqueues a `pending_scan_triggers` row (`trigger_type='page_speed'`); `api.cron.measure-page-speed` calls PSI on its own 60s invocation and patches the violation row. Inline, behind a 30s abort, PSI failed on ~2 of every 3 scans (17 of 26 attempts unmeasured, 16 of those timeouts, **zero** 429s — never a quota or key problem). **Cache-warming was refuted, not untried**: sex-eshop timed out twice 11 SECONDS apart and 7wf1na-x2 five times at 2-4 minute intervals; success tracks the STORE (8 of 13 shops measure first try, 5 never do). `scorable: false` on EVERY path is what makes the deferred patch safe — the denominator is 11 regardless, so a late result cannot move a score the merchant already saw. `raw_data.elapsed_ms` is recorded on every outcome including failures. **Known trade-off:** the interstitial signal comes from `lighthouseResult.audits["intrusive-interstitials"]` inside the same PSI response and is NOT separable (the module gets only a URL; no HTML-based detector exists), so intrusive interstitials are now advisory too. Follow-up: an HTML detector in `checks/shared/html-detectors.server.ts` would be fast, reliable and independently scorable. The CLI scanner still calls PSI inline — it has no 60s ceiling.
...
### 6.2 One-time plan selection redirect (2026-08-03)

**Shopify App Pricing does NOT show the plan picker on its own.** Its docs put that redirect on the app: *"When merchants install your app or need to select a plan, redirect them to this page."* ShieldKit never did it, so Shopify auto-enrolled every install on the Free plan and the paid plan was reachable only by clicking an in-app Upgrade button.

The evidence was unambiguous: months of Partner App-events are a solid wall of `Free - Free subscription` activations, and on 2026-08-03 the DB held **57 free merchants against 2 real payers**. Nobody was declining Monitoring — nobody was ever shown it.

Implemented in the `app.tsx` **layout** loader (so it fires regardless of which `/app/*` route the merchant lands on), gated on `!hasPaidAccess(tier) && plan_selection_shown_at IS NULL`.

**Deliberately NOT the documented `billing.check()` gate.** That pattern redirects whenever there is no active payment, which for an app with a real free tier bounces every free merchant to the picker on EVERY page load and makes the free tier unusable. (`billing.check()` is also unavailable here — no `billing` config is registered and it stopped reporting managed-pricing status after 2026-04-28.) The picker is shown ONCE; staying on Free is then a real choice.

**Ordering is load-bearing — build URL → stamp → redirect:**
- `getManagedPricingUrl` throws when `SHOPIFY_APP_HANDLE` is unset, and this is the LAYOUT loader: an uncaught throw takes out every `/app/*` route, not just this feature. Hence the try/catch, and hence building the URL before writing anything.
- **Stamping BEFORE the redirect is what makes a loop impossible.** A failed stamp therefore suppresses the redirect. A missed picker costs one click on Manage plan; a loop locks the merchant out of the app entirely. Fail toward the app.

`tests/plan-selection-redirect.test.ts` drives the real loader; three of its cases are the must-NOT-redirect ones.
...
|-----------|----------|-----------|
| `app.tsx` | `/app` (layout) | Wraps `/app/*`. NavMenu: Dashboard + Manage plan always; Pro Settings + AI bot toggle + Appeal letter + GTIN auto-filler when `hasPaidAccess(tier)` (GTIN additionally requires `WRITE_METAFIELDS_SCOPE_ENABLED`). **Also owns the one-time plan-selection redirect** (2026-08-03) — free tier + `plan_selection_shown_at IS NULL` → stamp the row, then `redirect(getManagedPricingUrl(shop), { target: "_top" })`. See §6.2. |
...
| `api.cron.measure-page-speed.ts` | `/api/cron/measure-page-speed` | **GET + POST** | GH Actions `*/30` + Vercel Cron daily 01:00 UTC failsafe. Drains `trigger_type='page_speed'`, runs PSI (45s budget), patches the violation row, recomputes `scans.passed_checks`/`*_count` — **never `compliance_score` or `total_checks`**. Deliberately NOT the enrichment drainer: a 45s PSI call would starve paying merchants' enrichment, and that drainer's SELECT is PAID-only while page_speed is part of the free scan. Bearer `CRON_SECRET`. |
...
* **Never write a whole JSONB column from a value read earlier in the request** (2026-07-30). `generatePolicy` read `generated_policies`, spread it in memory, and wrote the whole object back, so two concurrent generations of *different* policy types both started from the same base and the second silently dropped the first. `useSingleFlight` does not help — it is per-button, and these are different buttons. Write the single key with `jsonb_set` in an RPC (`set_generated_policy`). The same reasoning applies to `pro_settings` and `policy_regen_used`: **any whole-column JSONB write is a lost update waiting for a second concurrent writer.**
...
* **An enforcement path must never FALL THROUGH on a failed check** (2026-08-03). Both scan entry points handled a `decrement_scan_quota` RPC error by returning 402 only for an already-exhausted merchant and then continuing — running the scan while leaving the quota untouched. That silently converts the one-scan free tier into an unlimited one, and does it in the direction nobody investigates: the merchant gets their scan, the response is 200, no counter moves. It is §11a in the billing layer — *"we could not consume the quota"* became *"no quota needed consuming"*. The production signature was 47 free merchants holding a full `scans_remaining = 1` after running 98 scans between them, 33 with `scans_reset_at` still exactly equal to `created_at` (so nothing reset them — the decrement never ran). Both sites now refuse the scan with a 500. **Note the fail-direction inverts:** FROZEN subscriptions fail toward ACCESS because wrongly revoking a paid entitlement costs a customer; a failed quota check would wrongly GRANT free product, so it fails toward refusal. Decide which hazard applies before copying either pattern. Pinned by `tests/scan-quota-fallback.test.ts`.
...
* **Count on 2026-08-03:** **762 / 762 passing across 42 files** (added `plan-selection-redirect.test.ts`, `scan-quota-fallback.test.ts` — both proven to fail against the pre-fix source before landing). Previously 747/40 on 2026-07-30. (Was 329/14 on 2026-07-09 — the §14 file list above is stale beyond the nine it names; trust `ls tests/`.) The two 2026-07-30 batches added `sentry-flush.test.ts`, `scan-quota-refund.test.ts`, `scan-failure-visibility.test.ts`, `violations-scorable.test.ts`, `generated-policies-lost-update.test.ts`, `typecheck-gate.test.ts`, `page-speed-deferred.test.ts`.
* **`npm run typecheck` is at ZERO, down from 44 — and CI now enforces it.** `.github/workflows/ci.yml` runs `npm ci`, `npm run typecheck` and `npm test` on every pull request. Before 2026-07-30 NOTHING ran either on a PR (Vercel's build does not typecheck), which is how 44 errors accumulated unnoticed. The threshold is zero, not a baseline diff — a baseline diff is what you are forced into when the gate is already red. The 44 were `@shopify/polaris-types` being wrong about this codebase — `s-card` undeclared, refs typed `Ref<Button>` against an invariant `RefObject<HTMLElement|null>`, and an `IconType` union **stale against `custom-elements.json` shipped in the same package**. `app/polaris-jsx.d.ts` supersedes it (dropped from `tsconfig.types`); **no component was changed**. It is deliberately narrow — only the ten tags in use, `tone` pinned to the real manifest union — so it cannot become a blanket `any`. `tests/typecheck-gate.test.ts` pins that narrowness.
* **The last error was a REAL bug, and was fixed rather than silenced** (2026-07-30): `PlanStatusCard` passed `tone="subdued"` at `:173`/`:180`. `subdued` is a valid `background`/`borderColor` token but has **never** been in any `tone` union, so Polaris fell back and the muted icon did not render muted. Now `neutral`. The lesson generalises: when a stale type package is superseded, widen the *gaps* and leave the *real* violations visible — `tests/typecheck-gate.test.ts` asserts `"subdued"` can never enter the tone union.
...
* **A test that greps a source file must strip comments first.** `tests/typecheck-gate.test.ts` asserts the ambient declarations are NOT a blanket `any` — and failed on the header comment quoting `[tag: string]: any` as the thing it avoids. Prose that documents an anti-pattern looks exactly like the anti-pattern to a regex.
* **Prefer a behavioural test that FORCES the failure over a source-text assertion, and prove it fails against the old code.** `tests/scan-quota-refund.test.ts` drives the real route against a mock implementing the actual RPC SQL; restoring the buggy absolute write fails 3 of its 8 cases. A file-content assertion that has never been seen to fail is not evidence.
...
* **`vercel.json` defines 5 Vercel Cron jobs** (the first four confirmed registered in the Vercel dashboard 2026-07-29; `measure-page-speed` added 2026-07-30):
  ...
  - `/api/cron/measure-page-speed` — daily 01:00 UTC (**failsafe**; primary is GH Actions `*/30`. Drains `pending_scan_triggers` rows of `trigger_type='page_speed'`, calls PSI on its own 60s invocation, patches the placeholder violation row. One row per invocation — each is a PSI call of up to 45s.)
