...
* **Count on 2026-07-30 (batch 2):** **738 / 738 passing across 39 files.** (Was 329/14 on 2026-07-09 — the §14 file list above is stale beyond the nine it names; trust `ls tests/`.) The two 2026-07-30 batches added `sentry-flush.test.ts`, `scan-quota-refund.test.ts`, `scan-failure-visibility.test.ts`, `violations-scorable.test.ts`, `generated-policies-lost-update.test.ts`, `typecheck-gate.test.ts`.
* **`npm run typecheck` is a real gate again: 1 error, down from 44.** The 44 were `@shopify/polaris-types` being wrong about this codebase — `s-card` undeclared, refs typed `Ref<Button>` against an invariant `RefObject<HTMLElement|null>`, and an `IconType` union **stale against `custom-elements.json` shipped in the same package**. `app/polaris-jsx.d.ts` supersedes it (dropped from `tsconfig.types`); **no component was changed**. It is deliberately narrow — only the ten tags in use, `tone` pinned to the real manifest union — so it cannot become a blanket `any`. `tests/typecheck-gate.test.ts` pins that narrowness.
* **The 1 remaining error is REAL and deliberately visible:** `PlanStatusCard.tsx:198` passes `tone="subdued"` (set at `:173`/`:180`). `subdued` is a valid `background`/`borderColor` value but has **never** been a valid `tone`; the intended muted tone is `neutral`. Widening `tone` to clear it would make the gate lie. **Do not silence it — fix the component.**
