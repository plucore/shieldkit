...
- `api.cron.process-scan-triggers.ts` (daily 12:00 UTC + GitHub Actions every 6h) drains `BATCH_SIZE=1`, runs `enrichProductMetafields` for enrichment rows, marks `processed_at`.
...
| `api.cron.process-scan-triggers.ts` | `/api/cron/process-scan-triggers` | POST | Drains the queue, `BATCH_SIZE=1`. Hit by GitHub Actions every 6h (primary, see §15) AND Vercel Cron daily 12:00 UTC (failsafe). Bearer `CRON_SECRET`. |
...
2. **`api.cron.process-scan-triggers.ts`** drains the queue one row per invocation (`BATCH_SIZE=1`). Each invocation runs ~5–12s for an enrichment row (well within the 60s ceiling).
